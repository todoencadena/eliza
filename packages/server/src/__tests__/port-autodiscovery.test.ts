...
describe('Port Checking Logic Tests', () => {
     ...
     */
    const findAvailablePort = async (startPort: number, host: string = '0.0.0.0'): Promise<number> => {
        ...
        throw new Error(`Could not find available port after ${MAX_ATTEMPTS} attempts starting from ${startPort}`);
    };
