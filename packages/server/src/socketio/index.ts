...
export class SocketIORouter {
  ...
  private handleGenericMessage(socket: Socket, data: GenericMessageData) {
    try {
      ...
      logger.error({ src: 'ws', socketId: socket.id, error: error instanceof Error ? error.message : String(error) }, 'Error processing message');
    }
  }
    ...
    try {
      ...
      if (!channelExists) {
        ...
        try {
          ...
          logger.error({ src: 'ws', socketId: socket.id, channelId, error: errorMessage }, 'Failed to auto-create channel');
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error({ src: 'ws', socketId: socket.id, error: errorMessage }, 'Error processing message');
