...
export function createSynthesisRouter(agents: Map<UUID, IAgentRuntime>): express.Router {
  ...
  router.post('/:agentId/audio-messages/synthesize', async (req, res) => {
    ...
    try {
      ...
      logger.error('[TTS] Error generating speech:', error instanceof Error ? error.message : String(error));
      sendError(
        res,
        500,
        'PROCESSING_ERROR',
  ...
  router.post('/:agentId/speech/generate', async (req, res) => {
    ...
    try {
      ...
      logger.error('[SPEECH GENERATE] Error generating speech:', error instanceof Error ? error.message : String(error));
      sendError(
        res,
        500,
        'PROCESSING_ERROR',
