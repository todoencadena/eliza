...
export function createAgentWorldsRouter(agents: Map<UUID, IAgentRuntime>): express.Router {
  ...
  router.get('/worlds', async (_req, res) => {
    try {
      ...
      logger.error('[WORLDS LIST] Error retrieving worlds:', error instanceof Error ? error.message : String(error));
      sendError(
        res,
        500,
        '500',
  ...
  const createWorldHelper = async (
    ...
    try {
      ...
      logger.error('[WORLD CREATE] Error creating world:', error instanceof Error ? error.message : String(error));
      sendError(
        res,
        500,
        '500',
  ...
  router.patch('/:agentId/worlds/:worldId', async (req, res) => {
    ...
    try {
      ...
      logger.error('[WORLD UPDATE] Error updating world:', error instanceof Error ? error.message : String(error));
      sendError(
        res,
        500,
        '500',
