...
describe('Agent Runs API', () => {
  ...
  const logs: LogEntry[] = [
    ...
    },
    {
      ...
      body: { runId, status: 'completed', messageId, roomId, entityId: agentId, endTime: baseTime + 3000, duration: 3000 },
    },
    ...
    },
    {
      ...
      body: { runId, action: 'REPLY', actionId: 'act-1', result: { success: true }, promptCount: 2 },
    },
    // Model call
    {
  ...
  it('GET /api/agents/:agentId/runs should return aggregated runs', async () => {
    const response = await fetch(`http://localhost:${port}/api/agents/${agentId}/runs?roomId=${roomId}`);
  ...
  it('GET /api/agents/:agentId/runs/:runId should return a timeline', async () => {
    const response = await fetch(`http://localhost:${port}/api/agents/${agentId}/runs/${runId}?roomId=${roomId}`);
    expect(response.status).toBe(200);
    const body = await response.json();
