...
export function createGroupMemoryRouter(
  ...
  router.delete('/groups/:serverId', async (req, res) => {
    ...
    try {
      ...
      logger.error('[GROUP DELETE] Error deleting group:', error instanceof Error ? error.message : String(error));
      sendError(
        res,
        500,
        'DELETE_ERROR',
  ...
  router.delete('/groups/:serverId/memories', async (req, res) => {
    ...
    try {
      ...
      logger.error('[GROUP MEMORIES DELETE] Error clearing memories:', error instanceof Error ? error.message : String(error));
      sendError(
        res,
        500,
        'DELETE_ERROR',
