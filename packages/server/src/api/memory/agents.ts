...
export function createAgentMemoryRouter(agents: Map<UUID, IAgentRuntime>): express.Router {
  ...
  router.get('/:agentId/rooms/:roomId/memories', async (req, res) => {
    ...
    try {
      ...
      logger.error('[MEMORIES GET] Error retrieving memories for room:', error instanceof Error ? error.message : String(error));
      sendError(
        res,
        500,
        '500',
  ...
  router.get('/:agentId/memories', async (req, res) => {
    ...
    try {
      ...
      logger.error(`[AGENT MEMORIES] Error retrieving memories for agent ${agentId}:`, error instanceof Error ? error.message : String(error));
      sendError(
        res,
        500,
        'MEMORY_ERROR',
  ...
  router.patch('/:agentId/memories/:memoryId', async (req, res) => {
    ...
    try {
      ...
      logger.error(`[MEMORY UPDATE] Error updating memory ${memoryId}:`, error instanceof Error ? error.message : String(error));
      sendError(
        res,
        500,
        'UPDATE_ERROR',
  ...
  router.delete('/:agentId/memories', async (req, res) => {
    try {
      ...
      logger.error('[DELETE ALL AGENT MEMORIES] Error deleting all agent memories:', error instanceof Error ? error.message : String(error));
      sendError(
        res,
        500,
        'DELETE_ERROR',
  ...
  router.delete('/:agentId/memories/all/:roomId', async (req, res) => {
    try {
      ...
      logger.error('[DELETE ALL MEMORIES] Error deleting all memories:', error instanceof Error ? error.message : String(error));
      sendError(
        res,
        500,
        'DELETE_ERROR',
  ...
  router.delete('/:agentId/memories/:memoryId', async (req, res) => {
    try {
      ...
      logger.error(`[DELETE MEMORY] Error deleting memory ${req.params.memoryId}:`, error instanceof Error ? error.message : String(error));
      sendError(
        res,
        500,
        'DELETE_ERROR',
