...
export function createMessagingCoreRouter(serverInstance: AgentServer): express.Router {
  ...
  (router as any).post('/submit', async (req: express.Request, res: express.Response) => {
    ...
    try {
      ...
      logger.error('[Messages Router /submit] Error submitting agent message:', error instanceof Error ? error.message : String(error));
  ...
  (router as any).post('/complete', async (req: express.Request, res: express.Response) => {
    ...
    try {
      ...
      logger.error('[Messages Router /notify-complete] Error notifying message complete:', error instanceof Error ? error.message : String(error));
  ...
  (router as any).post('/ingest-external', async (req: express.Request, res: express.Response) => {
    ...
    try {
      ...
      logger.error('[Messages Router /ingest-external] Error ingesting external message:', error instanceof Error ? error.message : String(error));
