    ...
    try {
      ...
      if (!defaultServer) {
        ...
        try {
          ...
          logger.warn({ src: 'db', error: sqlError instanceof Error ? sqlError.message : String(sqlError) }, 'SQL insert failed, trying ORM');
    ...
    try {
      ...
      this.app.use(
        ...
        (err: unknown, req: Request, res: Response, _next: express.NextFunction) => {
          ...
          const errorCode = err && typeof err === 'object' && 'code' in err && typeof err.code === 'number' ? err.code : 500;
          logger.error({ src: 'http', error: errorMessage, method: req.method, path: req.path }, 'API error');
          res.status(500).json({
            success: false,
            error: {
  ...
  public async start(config?: ServerConfig): Promise<void> {
    ...
    if (!config?.isTestMode) {
      ...
      try {
        ...
        const errorObj = error && typeof error === 'object' && 'code' in error ? error as { code?: string } : null;
        if (errorObj && errorObj.code === 'EADDRINUSE') {
