      ...
      for (const testFile of testFiles) {
        try {
          ...
          try {
            ...
            safeLogger.error(`Failed to import test file ${fileName}:`, importError instanceof Error ? importError.message : String(importError));
            safeLogger.info(
          ...
          safeLogger.error(`Error running tests from ${testFile}:`, error instanceof Error ? error.message : String(error));
          this.stats.failed++;
        }
      }
    } catch (error) {
      safeLogger.error(`Error running e2e tests:`, error instanceof Error ? error.message : String(error));
    }
  }
