...
export async function installPluginWithSpinner(
  ...
  try {
    ...
    if (result.success) {
      ...
      logger.debug(`Plugin installation failed: ${packageName}`, JSON.stringify({
    ...
    logger.debug(`Plugin installation error: ${packageName}`, error instanceof Error ? error.message : String(error));
  }
}
