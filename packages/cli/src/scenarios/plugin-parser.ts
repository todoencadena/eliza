...
export class PluginParser {
    ...
    static async validatePlugins(plugins: ParsedPlugin[]): Promise<PluginValidationResult> {
        ...
        for (const plugin of plugins) {
            ...
            try {
                ...
                result.errors.push(`Error loading plugin '${plugin.name}': ${error instanceof Error ? error.message : String(error)}`);
                result.valid = false;
                continue;
            }
     ...
     */
    static async parseAndValidate(pluginReferences: PluginReference[] | undefined): Promise<PluginValidationResult> {
