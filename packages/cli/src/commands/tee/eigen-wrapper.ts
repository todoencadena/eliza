    ...
    .action(async (...commandArgs) => {
        ...
        if (!resolvedBinary) {
            ...
            if (!autoInstall && !isInteractive) {
                ...
                logger.error('   curl -fsSL https://eigenx-scripts.s3.us-east-1.amazonaws.com/install-eigenx.sh | bash');
                process.exit(1);
                return;
            }
            ...
            logger.info({ binary: resolvedBinary.command }, 'Eigen CLI installation completed successfully.');
        }
