...
describe('Report Generation Integration', () => {
    ...
    test('should handle directory with no valid run files', async () => {
        ...
        await expect(executeGenerateCommand(inputDir, { outputPath }))
            .rejects.toThrow();
    });
    ...
    test('should error on non-existent input directory', async () => {
        ...
        await expect(executeGenerateCommand(nonExistentDir, { outputPath }))
            .rejects.toThrow(`Input directory not found: ${nonExistentDir}`);
    });
