...
export class AnalysisEngine {
    ...
    private calculateSummaryStats(runs: ScenarioRunResult[]): ReportSummaryStats {
        ...
        const averageTotalTokens = runs.reduce((sum, run) => sum + run.metrics.total_tokens, 0) / totalRuns;
    ...
    private analyzeTrajectoryPatterns(runs: ScenarioRunResult[]): CommonTrajectory[] {
        const trajectoryPatterns: Map<string, {
            count: number;
        ...
        const totalRunsWithTrajectories = runs.filter(run => run.trajectory && run.trajectory.length > 0).length;
