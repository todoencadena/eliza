...
async function createHtmlReport(jsonReportPath: string, outputHtmlPath: string) {
    try {
        ...
        console.log(`   • ${(reportData.summary_stats.overall_success_rate * 100).toFixed(1)}% success rate`);
        console.log(`   • ${reportData.summary_stats.average_execution_time.toFixed(2)}s average execution time`);
