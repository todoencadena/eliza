...
class EnhancedStringContainsEvaluator implements EnhancedEvaluator {
    async evaluateEnhanced(params: EvaluationSchema, runResult: ExecutionResult): Promise<EnhancedEvaluationResult> {
...
class EnhancedFileExistsEvaluator implements EnhancedEvaluator {
    async evaluateEnhanced(params: EvaluationSchema, runResult: ExecutionResult): Promise<EnhancedEvaluationResult> {
        ...
        const success = createdFiles.includes(expectedPath) ||
            createdFiles.includes(`./${expectedPath}`) ||
            createdFiles.includes(expectedPath.replace(/^\.\//, ''));

        const matchingPath = createdFiles.find(path =>
            path === expectedPath ||
            path === `./${expectedPath}` ||
...
class EnhancedExecutionTimeEvaluator implements EnhancedEvaluator {
    async evaluateEnhanced(params: EvaluationSchema, runResult: ExecutionResult): Promise<EnhancedEvaluationResult> {
        ...
        if (duration == null || Number.isNaN(duration) || (runResult.durationMs === undefined && (runResult.startedAtMs === undefined || runResult.endedAtMs === undefined))) {
            return {
    ...
    async evaluateEnhanced(
        ...
        try {
            ...
            const actionResults = actionMemories.filter(
                (mem) => mem.content?.type === 'action_result'
            );
            ...
            const allActionNames = actionResults.map(mem => (mem.content as any)?.actionName || 'unknown');
