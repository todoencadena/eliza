...
export class RunDataAggregator {
    ...
    public startRun(
        ...
        this.runtime.logger.debug(`[RunDataAggregator] Started run ${runId} with combination ${combinationId}`);
    }
    ...
    public recordFinalResponse(response: string): void {
        ...
        this.runtime.logger.debug(`[RunDataAggregator] Recorded final response (${response.length} chars)`);
    }
    ...
    public async buildResult(
        ...
        if (!this.error) {
            try {
                ...
                this.runtime.logger.debug(`[RunDataAggregator] Collected ${trajectory.length} trajectory steps`);
            } catch (trajectoryError) {
                this.runtime.logger.warn(`[RunDataAggregator] Failed to collect trajectory: ${trajectoryError}`);
        ...
        if (!this.error && evaluations.length > 0) {
            try {
                ...
                this.runtime.logger.debug(`[RunDataAggregator] Collected ${evaluationResults.length} evaluation results`);
            } catch (evaluationError) {
                this.runtime.logger.warn(`[RunDataAggregator] Failed to collect evaluations: ${evaluationError}`);
