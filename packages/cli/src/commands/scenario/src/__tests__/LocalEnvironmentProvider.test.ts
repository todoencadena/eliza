...
describe('LocalEnvironmentProvider', () => {
    ...
    describe('Error Handling', () => {
        it('should throw error when run is called before setup', async () => {
            ...
            await expect(freshProvider.run(sampleScenario))
                .rejects.toThrow('Setup must be called before run.');
        });
        ...
        it('should throw error for steps with neither input nor code', async () => {
            ...
            await expect(provider.run(invalidScenario))
                .rejects.toThrow('Step must have either input or code');
        });
    ...
    describe('Agent Communication Requirements', () => {
        it('should require server and agent for natural language input', async () => {
            ...
            await expect(providerWithoutServer.run(scenario))
                .rejects.toThrow('LocalEnvironmentProvider requires a pre-created server and agent for NL input');
        });
    });
