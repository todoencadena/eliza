...
describe('Parameter Override System', () => {
    ...
    describe('Single Parameter Override', () => {
        it('should override simple string properties', () => {
            const overrides: ParameterOverride[] = [
                { path: 'name', value: 'Modified Test Scenario' }
            ];
        ...
        it('should override array elements', () => {
            const overrides: ParameterOverride[] = [
                { path: 'run[0].input', value: 'modified input' }
            ];
    ...
    describe('Error Handling', () => {
        it('should throw error for invalid parameter paths', () => {
            const overrides: ParameterOverride[] = [
                { path: 'nonexistent.path', value: 'test' }
            ];

            expect(() => applyParameterOverrides(baseScenario, overrides)).toThrow('Invalid parameter path');
        });

        it('should throw error for out-of-bounds array access', () => {
            const overrides: ParameterOverride[] = [
                { path: 'run[10].input', value: 'test' }
            ];

            expect(() => applyParameterOverrides(baseScenario, overrides)).toThrow('Array index out of bounds');
        });

        it('should throw error for invalid array index', () => {
            const overrides: ParameterOverride[] = [
                { path: 'run[invalid].input', value: 'test' }
            ];
        ...
        it('should provide helpful error messages', () => {
            const overrides: ParameterOverride[] = [
                { path: 'character.nonexistent', value: 'test' }
            ];
