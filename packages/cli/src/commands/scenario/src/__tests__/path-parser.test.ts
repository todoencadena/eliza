...
describe('Path Parser', () => {
    ...
    describe('parseParameterPath', () => {
        ...
        it('should throw error for malformed paths', () => {
            expect(() => parseParameterPath('.character.name')).toThrow('Path cannot start or end with a dot');
            expect(() => parseParameterPath('character..name')).toThrow('Empty segment in path');
            expect(() => parseParameterPath('character.name.')).toThrow('Path cannot start or end with a dot');
        });

        it('should throw error for unclosed brackets', () => {
            expect(() => parseParameterPath('run[0.input')).toThrow('Missing closing bracket for array index');
        });
    ...
    describe('reconstructPath', () => {
        ...
        it('should reconstruct array paths', () => {
            ...
            expect(reconstructPath(['plugins', 1, 'config', 'settings', 2])).toBe('plugins[1].config.settings[2]');
        });
