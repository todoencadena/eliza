...
describe('RunDataAggregator', () => {
    ...
    describe('buildResult', () => {
        it('should build a complete ScenarioRunResult for successful run', async () => {
            ...
            expect(mockEvaluationEngine.runEnhancedEvaluations).toHaveBeenCalledWith(evaluations, executionResult);
        });
        ...
        it('should throw error if run not started', async () => {
            ...
            await expect(
                aggregator.buildResult('room-id', [], {})
            ).rejects.toThrow('Run not started. Call startRun() first.');
        });
    });
