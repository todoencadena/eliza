...
export class TaskService extends Service {
  ...
  async createTestTasks() {
    ...
    this.runtime.registerTaskWorker({
      ...
      validate: async (_runtime, _message, _state) => {
        logger.debug({ src: 'plugin:bootstrap:service:task', agentId: this.runtime.agentId }, 'Validating repeating test task');
        return true;
      },
      execute: async (_runtime, _options) => {
        logger.debug({ src: 'plugin:bootstrap:service:task', agentId: this.runtime.agentId }, 'Executing repeating test task');
      },
    });
    ...
    this.runtime.registerTaskWorker({
      ...
      validate: async (_runtime, _message, _state) => {
        logger.debug({ src: 'plugin:bootstrap:service:task', agentId: this.runtime.agentId }, 'Validating one-time test task');
        return true;
      },
      execute: async (_runtime, _options) => {
        logger.debug({ src: 'plugin:bootstrap:service:task', agentId: this.runtime.agentId }, 'Executing one-time test task');
      },
    });
  ...
  private startTimer() {
    ...
    this.timer = setInterval(async () => {
      try {
        ...
        logger.error({ src: 'plugin:bootstrap:service:task', agentId: this.runtime.agentId, error: error instanceof Error ? error.message : String(error) }, 'Error in task timer');
      }
      ...
      for (const task of tasks) {
        ...
        if (task.metadata?.updatedAt === task.metadata?.createdAt) {
          if (task.tags?.includes('immediate')) {
            logger.debug({ src: 'plugin:bootstrap:service:task', agentId: this.runtime.agentId, taskName: task.name }, 'Immediately running task');
            await this.executeTask(task);
            continue;
          }
        }
        ...
        if (now - taskStartTime >= updateIntervalMs) {
          logger.debug({ src: 'plugin:bootstrap:service:task', agentId: this.runtime.agentId, taskName: task.name, intervalMs: updateIntervalMs }, 'Executing task - interval elapsed');
          await this.executeTask(task);
        }
      }
    } catch (error) {
      logger.error({ src: 'plugin:bootstrap:service:task', agentId: this.runtime.agentId, error: error instanceof Error ? error.message : String(error) }, 'Error checking tasks');
    }
  }
  ...
  private async executeTask(task: Task) {
    try {
      if (!task || !task.id) {
        logger.debug({ src: 'plugin:bootstrap:service:task', agentId: this.runtime.agentId }, 'Task not found');
        return;
      }

      const worker = this.runtime.getTaskWorker(task.name);
      if (!worker) {
        logger.debug({ src: 'plugin:bootstrap:service:task', agentId: this.runtime.agentId, taskName: task.name }, 'No worker found for task type');
        return;
      }
      ...
      if (task.tags?.includes('repeat')) {
        ...
        logger.debug({ src: 'plugin:bootstrap:service:task', agentId: this.runtime.agentId, taskName: task.name, taskId: task.id }, 'Updated repeating task with new timestamp');
      }

      logger.debug({ src: 'plugin:bootstrap:service:task', agentId: this.runtime.agentId, taskName: task.name, taskId: task.id }, 'Executing task');
      ...
      if (!task.tags?.includes('repeat')) {
        ...
        logger.debug({ src: 'plugin:bootstrap:service:task', agentId: this.runtime.agentId, taskName: task.name, taskId: task.id }, 'Deleted non-repeating task after execution');
      }
    } catch (error) {
      logger.error({ src: 'plugin:bootstrap:service:task', agentId: this.runtime.agentId, taskId: task.id, error: error instanceof Error ? error.message : String(error) }, 'Error executing task');
    }
  }
