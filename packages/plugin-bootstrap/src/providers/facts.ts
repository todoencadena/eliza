...
const factsProvider: Provider = {
  ...
  get: async (runtime: IAgentRuntime, message: Memory, _state?: State) => {
    try {
      ...
      logger.error({ src: 'plugin:bootstrap:provider:facts', agentId: runtime.agentId, error: error instanceof Error ? error.message : String(error) }, 'Error in factsProvider');
      return {
        values: {
          facts: '',
        },
