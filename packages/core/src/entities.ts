...
export async function findEntityByName(
  ...
  let matchesArray: Array<{ name?: string; reason?: string }> = [];
  const matches = resolution.matches as { match?: unknown } | undefined;
  if (matches?.match) {
    // Normalize to array
    const matchValue = matches.match;
    matchesArray = Array.isArray(matchValue)
      ? (matchValue as Array<{ name?: string; reason?: string }>)
      : [matchValue as { name?: string; reason?: string }];
  }
