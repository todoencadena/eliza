...
describe('crypto-compat', () => {
    describe('createHashAsync (cross-platform)', () => {
        ...
        it('should throw on unsupported algorithm', async () => {
            await expect(
                createHashAsync('invalid-hash-algo' as any, 'data')
    ...
    describe('encryptAsync / decryptAsync (cross-platform)', () => {
        ...
        it('should throw on invalid key length', async () => {
            ...
            await expect(
                encryptAsync(invalidKey, iv, data)
            ).rejects.toThrow(/invalid key length/i);

            await expect(
                decryptAsync(invalidKey, iv, data)
            ).rejects.toThrow(/invalid key length/i);
        });
        ...
        it('should throw on invalid IV length', async () => {
            ...
            await expect(
                encryptAsync(key, invalidIV, data)
            ).rejects.toThrow(/invalid iv length/i);

            await expect(
                decryptAsync(key, invalidIV, data)
            ).rejects.toThrow(/invalid iv length/i);
        });
    ...
    describe('error messages', () => {
        ...
        it('should validate algorithm names', async () => {
            await expect(
                createHashAsync('invalid-algo' as any, 'data')
            ).rejects.toThrow();
        });
