...
export class ServiceBuilder<TService extends Service = Service> {
  ...
  build(): {
    ...
    const ServiceClassWithType = ServiceClass as unknown as typeof ServiceClass & {
      new (runtime?: IAgentRuntime): TService;
      serviceType: ServiceTypeName;
      start(runtime: IAgentRuntime): Promise<TService>;
    };
    // Ensure serviceType is properly typed - override the static property
    (ServiceClassWithType as { serviceType: ServiceTypeName }).serviceType = serviceType as ServiceTypeName;
    return ServiceClassWithType;
  }
}
