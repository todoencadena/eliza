...
var MessageManager = class {
  ...
  async _downloadAttachments(event) {
    if (event.files == null || event.files.length == 0) {
      return event;
    }
    elizaLogger.log("\u{1F4E5} Downloading attachments");
  ...
  async _uploadAttachments(event, attachments) {
    ...
    try {
      ...
      try {
        ...
        const memory = {
          ...
          roomId,
          content,
          createdAt: new Date(Number.parseFloat(event.ts) * 1e3).getTime(),
          ...
          if (responseContent?.text) {
            ...
            const callback = async (content2, attachments) => {
              try {
                ...
                const messageText = content2.text || responseContent.text;
        ...
        console.log(
          "\u{1F513} Final Step: Removing message from processing lock and deleting downloaded attachments"
        );
...
var slackEnvSchema = z.object({
  ...
  SLACK_SERVER_PORT: z.string().optional().transform((val) => val ? Number.parseInt(val) : 3e3)
});
