...
describe('TEE Frontend Components', () => {
    ...
    describe('Fetch with Retry Logic', () => {
        const fetchWithRetry = async (url: string, options: { timeout?: number; retries?: number } = {}) => {
        ...
        test('should fail after max retries', async () => {
            ...
            await expect(fetchWithRetry('/mr-tee-status', { retries: 2 }))
        ...
        test('should handle HTTP error status codes', async () => {
            ...
            await expect(fetchWithRetry('/mr-tee-status', { retries: 1 }))
...
describe('TEE Frontend Performance', () => {
    ...
    test('should handle concurrent requests gracefully', async () => {
        ...
        const requests = Array(5).fill(null).map(() =>
            fetch('/mr-tee-status').then(r => r.json())
        );
