  ...
  async start() {
    const handleInteractionsLoop = async () => {
      ...
      this.timeout = setTimeout(
        ...
        Number(this.client.farcasterConfig?.FARCASTER_POLL_INTERVAL ?? 120) * 1e3
...
var FarcasterClientInterface = {
  ...
  async stop(runtime) {
    try {
      elizaLogger5.log("Stopping farcaster client", runtime.agentId);
      if (runtime.clients.farcaster) {
        await runtime.clients.farcaster.stop();
      }
    } catch (e) {
      elizaLogger5.error("client-farcaster interface stop error", e);
    }
  }
};
