...
export class DrizzleSchemaIntrospector {
      ...
      for (const [index, fk] of inlineForeignKeys.entries()) {
        ...
        if (fk && fk.reference && typeof fk.reference === 'function') {
          try {
            ...
            logger.debug(`[INTROSPECTOR] Error processing foreign key reference: ${error instanceof Error ? error.message : String(error)}`);
          }
        }
      }
    } else {
  ...
  private extractReferencedTableName(reference: any): string | null {
    logger.debug(`[INTROSPECTOR] Extracting referenced table name from: ${JSON.stringify({
    ...
    if (typeof reference === 'function') {
      try {
        ...
        logger.debug(`[INTROSPECTOR] Error calling reference function: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  ...
  private parseIndexes(table: any): IndexDefinition[] {
      ...
      try {
        ...
        if (Array.isArray(extraConfig)) {
          ...
          for (const item of extraConfig) {
            logger.debug(`[INTROSPECTOR] ExtraConfig item: ${JSON.stringify({
        ...
        logger.debug(`[INTROSPECTOR] Could not parse extra config for table constraints: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
    ...
    if (indexes.length === 0) {
      ...
      try {
        ...
        logger.debug(`[INTROSPECTOR] Error checking symbols: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
    ...
    logger.debug(`[INTROSPECTOR] Found ${indexes.length} indexes/constraints: ${JSON.stringify(indexes)}`);
    return indexes;
  }
  ...
  private parseCheckConstraints(table: any): { name: string; expression: string }[] {
    ...
    if (tableConfig && tableConfig.extraConfigBuilder) {
      try {
        ...
        logger.debug(`[INTROSPECTOR] Could not parse check constraints: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  ...
  private parseCompositePrimaryKey(table: any): { name: string; columns: string[] } | undefined {
    ...
    if (tableConfig && tableConfig.extraConfigBuilder) {
      try {
        ...
        logger.debug(`[INTROSPECTOR] Could not parse composite primary key: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  ...
  try {
    ...
    for (const tableName of sortedTableNames) {
      ...
      if (tableDef.foreignKeys.length > 0 || tableDef.checkConstraints.length > 0) {
        logger.debug(`[CUSTOM MIGRATOR] Adding constraints for table: ${tableDef.name} - ${JSON.stringify({
