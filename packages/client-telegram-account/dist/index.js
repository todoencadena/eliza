...
// src/environment.ts
var telegramAccountEnvSchema = z.object({
...
async function validateTelegramAccountConfig(runtime) {
  try {
    const telegramAccountConfig = {
      ...
      TELEGRAM_ACCOUNT_APP_ID: safeParseInt(
        runtime.getSetting("TELEGRAM_ACCOUNT_APP_ID") || process.env.TELEGRAM_ACCOUNT_APP_ID
      ),
      TELEGRAM_ACCOUNT_APP_HASH: runtime.getSetting("TELEGRAM_ACCOUNT_APP_HASH") || process.env.TELEGRAM_ACCOUNT_APP_HASH,
      TELEGRAM_ACCOUNT_DEVICE_MODEL: runtime.getSetting("TELEGRAM_ACCOUNT_DEVICE_MODEL") || process.env.TELEGRAM_ACCOUNT_DEVICE_MODEL,
      TELEGRAM_ACCOUNT_SYSTEM_VERSION: runtime.getSetting("TELEGRAM_ACCOUNT_SYSTEM_VERSION") || process.env.TELEGRAM_ACCOUNT_SYSTEM_VERSION
    };
...
var telegramAccountMessageHandlerTemplate = `
{{actionExamples}}
...
function getTelegramAccountMessageHandlerTemplate(account) {
...
// src/telegramAccountClient.ts
var TelegramAccountClient = class {
  runtime;
  telegramAccountConfig;
  client;
  account;
  constructor(runtime, telegramAccountConfig) {
    elizaLogger.log("\u{1F4F1} Constructing new TelegramAccountClient...");
    this.runtime = runtime;
    this.telegramAccountConfig = telegramAccountConfig;
    elizaLogger.log("\u2705 TelegramClient constructor completed");
  }
  async start() {
    elizaLogger.log("\u{1F680} Starting Telegram account...");
    try {
  ...
  async initializeAccount() {
    ...
    this.account = await this.client.getEntity("me");
  }
  setupEventsHandlers() {
  ...
  newMessageHandler() {
    this.client.addEventHandler(async (event) => {
      try {
        ...
        const userUUID = stringToUuid(`tg-${sender.id.toString()}`);
        const roomUUID = stringToUuid(`tg-${chat.id.toString()}-` + this.runtime.agentId);
        const messageUUID = stringToUuid(`tg-message-${roomUUID}-${event.message.id.toString()}-` + this.runtime.agentId);
        const agentUUID = this.runtime.agentId;
        const replyMessageUUID = replyMessage ? stringToUuid(`tg-message-${roomUUID}-${replyMessage.id.toString()}-` + this.runtime.agentId) : null;
        ...
        const memory = {
          ...
          roomId: roomUUID,
          content,
          createdAt: event.message.date * 1e3,
        ...
        const callback = async (content2) => {
          ...
          if (sentMessages) {
            ...
            for (let i = 0; i < sentMessages.length; i++) {
              ...
              const memory2 = {
                id: stringToUuid(`tg-message-${roomUUID}-${sentMessage.id.toString()}-` + this.runtime.agentId),
                agentId: agentUUID,
                userId: agentUUID,
                roomId: roomUUID,
                content: {
        ...
        if (shouldRespond) {
          ...
          const template = this.runtime.character?.templates?.messageHandlerTemplate || getTelegramAccountMessageHandlerTemplate(this.account);
  ...
  async _shouldRespond(message, chat, replyMessage) {
    if (replyMessage) {
      const replyFrom = replyMessage.fromId;
  ...
  async sendMessageInChunks(chatId, content, replyToMessageId) {
...
var TelegramAccountClientInterface = {
  ...
  stop: async (_runtime) => {
    elizaLogger2.warn("Telegram client does not support stopping yet");
  }
};
var index_default = TelegramAccountClientInterface;
export {
